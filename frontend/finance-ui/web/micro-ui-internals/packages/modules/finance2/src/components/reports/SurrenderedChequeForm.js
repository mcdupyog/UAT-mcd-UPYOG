...
const SurrenderedChequeForm = ({ onSubmit, onReset }) => {
  ...
  const handleSubmit = useCallback(() => {
    if (!formData.fundName || !formData.bankAccount || !formData.bankBranch) {
    ...
    <Card style={{ padding: "16px", marginBottom: "20px" }}>
      ...
      >
        ...
        <div>
          ...
          <FormField label="Reason For Surrender:" isMandatory>
            <CustomDropdown
              t={t}
              config={CONFIG.reasonForSurrender.populators}
              onChange={(e) => setFormData({ ...formData, reasonForSurrender: e })}
              value={formData.reasonForSurrender}
            />
          </FormField>
...
export default SurrenderedChequeForm;
