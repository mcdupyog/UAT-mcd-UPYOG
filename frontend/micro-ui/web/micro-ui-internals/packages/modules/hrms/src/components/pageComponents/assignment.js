  ...
  const Assignments = ({ t, config, onSelect, userType, formData }) => {
    ...
    function getDivisionData(department, assignmentKey) {
      const mappings = commonData?.["common-masters"]?.ZoneDivisionMapping || [];
      const jurisdiction = formData?.Jurisdictions?.[0]; 
      const selectedZone = jurisdiction?.zone;
      if (!department || !selectedZone) return [];
      const mapping = mappings.find(
        (m) =>
          m.zoneCode === selectedZone &&
          m.departmentCode === department.code
      );
      return mapping
  ...
  function Assignment({
      ...
      setassignments((pre) =>
        pre.map((item) =>
          item.key === assignment.key
            ? { ...item, department: value, division: null }
            : item
        )
      );
    };
    ...
    const selectDivision = (value) => {
      setassignments((pre) =>
        pre.map((item) =>
          item.key === assignment.key ? { ...item, division: value } : item
        )
      );
    };
        ...
        <div style={{ border: "1px solid #E3E3E3", padding: "16px", marginTop: "8px" }}>
          ...
          <LabelFieldPair>
            ...
            <Dropdown
              ...
              disable={!assignment?.department || getdivisiondata(assignment?.department, assignment.key).length === 0}
              optionKey={"i18key"}
